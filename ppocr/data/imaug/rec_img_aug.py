...
class SATRNRecResizeImg(object):
    ...
    def __call__(self, data):
        img = data['image']
        norm_img, resize_shape, pad_shape, valid_ratio = resize_norm_img_satrn(
...
def resize_norm_img_satrn(img, image_shape):
    imgC, imgH, imgW = image_shape
    h = img.shape[0]
    w = img.shape[1]
    valid_ratio = 1.0
    ...
    pad_shape = resized_image.shape

    return resized_image, resize_shape, pad_shape, valid_ratio
